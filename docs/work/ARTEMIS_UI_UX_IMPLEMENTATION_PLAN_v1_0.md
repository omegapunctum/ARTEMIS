...
>
> Main-screen de-island alignment sync (2026-04-26): de-island visual alignment cycle принят как **current working baseline** главного экрана. Baseline ориентирован на integrated docked workstation layout (а не floating glass-card islands) и включает: docked top shell + slice bar, docked right inspector panel, docked bottom timeline band, integrated left rail + MapLibre controls, map-first hierarchy preserved. Это **не** финальное visual completion; дальнейшие изменения main-screen допускаются только через audit-first.
>
> Main-screen structural de-island sync (2026-04-26): после workspace-frame refactor и final structural audit принят **current structural baseline accepted** для main screen. Рабочий contour теперь фиксирован как `workspace-frame / docked-grid`: top dock (`#workspace-top-dock`), main grid (rail / map / optional inspector), bottom dock (`#workspace-bottom-dock`); mobile detail намеренно остаётся bottom-sheet (fixed), а transient overlays могут сохранять glass/fixed-паттерны. Это **не** заявление о final visual completion; дальнейшие main-screen изменения остаются только audit-first.
