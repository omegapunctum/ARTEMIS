...
> Lifecycle: gated compatibility context. `docs/work/README.md` is authoritative; this file is not an active execution order.
...
>
> Main-screen refinement sub-track sync (2026-07-16): активный audit-backed sub-track задаётся `docs/work/uiux/2026-07-16_UIUX_MAIN_SCREEN_REFINEMENT_SPEC_ACTIVE_v1_0.md`. Scope: compact top shell/context, full-width map workspace, overlay tools, on-demand detail geometry, compact timeline, icon consistency и CSS owner cleanup. Story/Course/Compare full-mode redesign и data/API changes в sub-track не входят.
>
> Main-screen geometry override sync (2026-07-16): апрельский integrated docked layout остаётся описанием current pre-patch runtime, но его structural left rail и nested framing **superseded как target**. Новый rollout сохраняет map-first hierarchy через full-width map и disciplined overlays.
>
