...
> Main-screen refinement track sync (2026-07-16): активный track-specific target главного экрана определён в `2026-07-16_UIUX_MAIN_SCREEN_REFINEMENT_SPEC_ACTIVE_v1_0.md`. Он фиксирует compact map-first shell, full-width map workspace, compact timeline и CSS owner cleanup, не заменяя owner-doc роли `ARTEMIS_UI_UX_SYSTEM.md` и `ARTEMIS_UI_UX_VISUAL_SYSTEM.md`. Переход к runtime patch остаётся audit-first.
>
> Main-screen geometry override sync (2026-07-16): апрельская модель `integrated left rail + docked-grid` сохраняет историческую ценность, но **superseded для следующего implementation cycle**. Текущий target удаляет structural desktop rail, упрощает outer frame/top dock и сохраняет tools как compact map overlay.
>
> Main-screen structural transition sync (2026-07-16): существующий `workspace-frame / docked-grid` описывает current runtime до patch, но не target geometry. Target contract: compact top shell, one-column map workspace, optional right inspector, compact bottom timeline; mobile detail остаётся intentional bottom-sheet.
>
> App-structure foundation sync (2026-04-26): created working app-structure spec `2026-04-26_UIUX_APP_STRUCTURE_SPEC_ACTIVE_v1_0.md` as expansion-planning layer for future UI/UX growth. It defines App Shell / Workspace Core / Product Sections / Shared Components / Shared Overlays / Feature Modules and Section Contract. It does not replace `ARTEMIS_UI_UX_SYSTEM.md` or `ARTEMIS_UI_UX_COMPONENT_MAP.md`; future tabs/sections must use this contract before runtime patches.
...
- помочь пользователю найти и доказательно сравнить архитектурные объекты;
- удержать контекст;
- показать динамику во времени;
- сохранить полезный контекст и позволить вернуться к нему;
- отделить reviewed relations от computed similarity и interpretation.
...
Первичный интерактивный объект ARTEMIS v1.0 — **sourced Architecture Feature в map-time context**. Первый analytical outcome — **сравнение 2–3 Features**. Главный сохраняемый объект — **Research Slice / Сохранённое исследование**.

## 2.2 Роль объекта и сравнения
Отдельный объект на карте является понятной точкой входа. Detail показывает sourced facts, provenance и доступные reviewed relations. Selection двух или трёх объектов создаёт основной момент исследовательской ценности: temporal, spatial, stylistic и source-aware comparison.

Object comparison не смешивается со сравнением сохранённых срезов. Primary action `Сравнить` относится к Features; secondary slice comparison при необходимости получает отдельное название.

## 2.3 Роль Slice
Research Slice сохраняет уже полезную конфигурацию:
...
- единицей возврата;
- единицей шаринга;
- контейнером comparison state и пользовательских заметок;
- основой для curated story context.

Slice не является обязательным первым действием. В пользовательском copy до validation применяется «Сохранённое исследование»; canonical internal name сохраняется.
...
1. Пользователь выбирает период, направление или регион либо находит объект через search.
2. Открывает sourced preview/detail.
3. Добавляет 2–3 Features в comparison selection.
4. Сравнивает время, место, направление, sources, reviewed Relations и separately labelled Similarity.
5. Сохраняет полезный контекст как Research Slice.
6. Возвращается к нему или делится read-only представлением.
...
5. Detail / comparison inspector
6. Contextual save/share overlays
...
Future section structure rule:
- new tabs/pages must be planned as sections through the app-structure spec;
- do not add future sections directly into index.html/style.css/ui.js without Section Contract;
- Workspace Core remains the primary runtime;
- Stories/Courses/Saved/Profile/About extend the product but do not become competing app centers.
