# Epistemic Conflict / Uncertainty Proof v1 — owner authorization and narrowed evidence gate

- Recorded: 2026-09-24; recording date does not assert a research or review date.
- Baseline: main `72a3ae34bf7d5aa70abf47b4577e65e537e34625` (#448).
...
- Initial execution state: **EVIDENCE_CLOSURE_REQUIRED** (historical until PR #451 acceptance). At the evidence closeout: [owner-accepted package / implementation-ready](2026-09-24_EPISTEMIC_CONFLICT_EVIDENCE_ACCEPTANCE_CLOSEOUT_v1.md); implementation had not started. Later [technical implementation closeout](2026-09-24_EPISTEMIC_CONFLICT_IMPLEMENTATION_CLOSEOUT_v1.md) records the non-public PR #453 merge and separate presentation acceptance.

Current owner decision (2026-09-24): **`NARROW_EPISTEMIC_CONFLICT_PROOF_V1`**; Product & UX disposition: **`SPEC_AMENDMENT_READY`**. The accepted bounded Product Specification and accepted Amendment are durably consolidated in [v1.1](2026-09-24_EPISTEMIC_CONFLICT_PROOF_PRODUCT_SPECIFICATION_v1_1.md). The original standalone text was absent from GitHub; v1.1 explicitly identifies its owner-provided handoff and narrowing provenance. This v1.1 record is the current bounded implementation authority. `implementation_started = false` at the earlier evidence closeout; current lifecycle is recorded in the [implementation closeout](2026-09-24_EPISTEMIC_CONFLICT_IMPLEMENTATION_CLOSEOUT_v1.md). Owner-accepted [evidence package v1](2026-09-24_ATL_1466_1_CONFLICT_EVIDENCE_PACKAGE_v1.md) and its exact revision are recorded in the [closeout](2026-09-24_EPISTEMIC_CONFLICT_EVIDENCE_ACCEPTANCE_CLOSEOUT_v1.md).

## Authorized bounded behavior

The owner-provided handoff of the accepted bounded specification was consolidated with the accepted narrowing into [v1.1](2026-09-24_EPISTEMIC_CONFLICT_PROOF_PRODUCT_SPECIFICATION_v1_1.md). It is not a claim that the unavailable original was reproduced verbatim. The owner-authorized boundary, provenance and non-goals are now inspectable from GitHub alone.

One canonical artifact identity must retain two separate, unresolved dating Claims, each with its own reviewed EvidenceLink/locator. The user should be able to see that the **dating is disputed**, which scholar supports each position, and the different chronological consequences without opening technical provenance. An attributed opinion must not be presented as an established historical fact.

Existing Claim/Evidence/Uncertainty, Explorer State and Render Projection contracts own meaning. No canonical winner, averaged/midpoint date, synthesized `1490–1515` duration, 50/50 weight, source score or automatic recency preference is authorized. Within-Claim approximate uncertainty and between-Claim disagreement remain distinct. The possible two-period use of the sheet is attributed and conditional, not a third Claim or proof that both dates are true.

Selecting the artifact selects it once. Interpretation focus is reversible presentation/query state, never canonical truth mutation. The accepted Amendment removes case-specific Range membership, Scrub entry/accumulation and normalized timeline filtering coordinates from v1. Existing Leonardo Range/Scrub remains unchanged. A fair same-content conventional scholarly prose comparator is required for later human evaluation. Neither browser evidence nor implementation demonstrates comparative user value.

The bounded implementation required a human presentation/interaction review before merge under `DEVELOPMENT_OPERATING_SYSTEM.md` §6.6; the owner review and PR #453 merge are recorded in the [technical closeout](2026-09-24_EPISTEMIC_CONFLICT_IMPLEMENTATION_CLOSEOUT_v1.md). Real novice comparison is still uncollected. No second case, fundamental Conflict type, generalized conflict system, source ranking, expanded corpus, new backend/Airtable, agent/MCP/memory/simulation, contextual composition or Global/Focus work is authorized.
...
3. two separately reviewed, appropriately scoped dating Claims for the exact bounded subject;
4. distinct reviewed EvidenceLinks, locators and review states for both Claims;
...
The next permitted action **at the earlier evidence closeout** was bounded Engineering implementation under [v1.1](2026-09-24_EPISTEMIC_CONFLICT_PROOF_PRODUCT_SPECIFICATION_v1_1.md); PR #453 subsequently merged and is recorded in the [technical closeout](2026-09-24_EPISTEMIC_CONFLICT_IMPLEMENTATION_CLOSEOUT_v1.md). Return to Command Center if the exact Claim subject, honest provenance or reviewed Claims/EvidenceLinks cannot be established under existing semantics. This decision record does **not** constitute the evidence package or authorize promotion before review.
