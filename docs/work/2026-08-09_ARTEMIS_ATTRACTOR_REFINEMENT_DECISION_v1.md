...
ARTEMIS keeps the technical term **World Model**, but Foundation v3.1 clarifies its epistemic meaning:
...
- `docs/SPATIOTEMPORAL_WORLD_MODEL_CONTRACT.md` — World Model vs external reality boundary;
...
No runtime, database, API, Airtable, ETL or World Slice migration is required.

The existing names `World Model`, `World Slice`, `Explorer State` and `Render Projection` remain valid. Existing fixtures do not need semantic rewrites solely because the epistemic boundary is now explicit.

Future contracts must interpret `World Model` under this clarified meaning.
...
3. `SPATIOTEMPORAL_WORLD_MODEL_CONTRACT.md` rejects objective-digital-twin/completeness interpretation;
...
8. Release Discipline Gate passes on the exact PR head.
