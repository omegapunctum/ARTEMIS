...
- `Leonardo Life Path v0` narrows the default Globe presentation to two understandable time modes over the same frozen material: `Range` shows all four source-bound stops inside a selected interval, while `Journey` progressively reveals accumulated stops from a chosen start. Selecting a numbered marker opens a compact place/date/activity card with source and uncertainty under progressive disclosure. The 96 Explorer views and layer semantics remain underlying technical evidence, not default controls.
...
- The current life-path presentation covers only those four 1502 stops. It does not claim a complete 1452–1519 biography, duration at each place or known routes between stops; a broader major-life-anchor package remains a separate data decision.
