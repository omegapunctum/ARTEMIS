...
The public root is a small ARTEMIS Core landing. It routes the browser-executed Leonardo Globe at `/globe/` as the primary research prototype and preserves the former Architecture Atlas at `/atlas/` as compatibility-only. Issue #355 remains the source-aware Globe MVP product vertical. Gate C is completed/FREEZE for the Leonardo-in-Romagna 1502 candidate boundary; Gate D is narrowed by Core Reset. The manual M5 whole-life check recorded `ITERATE`; one bounded UX correction may be scoped separately, but no successor branch is open. Public access does not make the Globe or its draft/rejected historical content product-validated; generative AI, personal knowledge, causal reasoning, VR/AR and universal coverage remain future/gated scope.

The current Globe loop is a bounded `Leonardo Life Path` interaction scaffold: use the full-width bottom timeline as either a two-handle calendar `Range` or a single-current-time `Scrub`, then select a documented presence. One click opens a compact map popup without moving the camera; further action opens the right detail drawer, while double-click may focus the map. M5 presents 11 reviewed coarse Presence anchors in six periods across 1452–1519; they are not a complete biography. Historical route geometry remains unknown and no route line is drawn.
