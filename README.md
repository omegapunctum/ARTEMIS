...
The public root is a small ARTEMIS Core landing. It routes the browser-executed Leonardo Globe at `/globe/` as the primary research prototype and preserves the former Architecture Atlas at `/atlas/` as compatibility-only. The Globe UI supports deterministic English and Russian presentation through `?lang=en|ru`; Claims, locators and Source records remain in their evidence language. Issue #355 remains the source-aware Globe MVP product vertical. Gate C is completed/FREEZE for the Leonardo-in-Romagna 1502 candidate boundary; Gate D is narrowed by Core Reset. Public access does not make the Globe or its draft/rejected historical content product-validated; generative AI, personal knowledge, causal reasoning, VR/AR and universal coverage remain future/gated scope.
