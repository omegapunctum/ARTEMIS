...
def test_agent_and_documentation_routing_have_single_registries() -> None:
    ...
    assert "RELATION_LADDER_CONTRACT.md" in foundation
    ...
    assert "validate_relation_fixtures.py" in structure
    assert "uncertainty/" in structure
    assert "relations/" in structure


def test_relation_contract_is_the_only_active_execution_order() -> None:
    ...
    assert "Resolve #331 relation-ladder semantics" in priorities
    ...
    assert "preserve completed #330 / PR #337 uncertainty semantics" in truth
